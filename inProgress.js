...
const scraperObject = {
  ...
  async function getFloorPlansLink(browser, fPurl){
    ...
    try {
      ...
      //await page.waitForSelector('.itemlist')
      try {
        let arrMainLinks = await page.$$('.itemlist .itemRow .row-list-data .div-row .title a');  
    ...
    try {
        ...
        const throttledProcess = async (items, interval) => { 
          ...
          try {
            ...
            const bb = await getFloorPlansLink(browser, items[0].floorPlanUrl)
            //console.log(bb)
            
            setTimeout(() => {
                getFloorPlansDetails(browser)
            }, 20000);
